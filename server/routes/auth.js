...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const patient = await createPatient({ name, email, phone: phone || null, password });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const patient = await verifyPatientPassword(email, password);
    if (!patient) {
...
router.get(
  ...
  requirePatient,
  asyncHandler(async (req, res) => {
    res.json({ data: await listBookingsForPatient(req.session.patient.id) });
  })
);
...
router.get(
  ...
  requirePatient,
  asyncHandler(async (req, res) => {
    res.json({ data: await listScreeningSubmissionsForPatient(req.session.patient.id) });
  })
);
